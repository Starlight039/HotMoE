...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            step = -1
            epoch_iterator = iter(epoch_dataloader) # 将 dataloader 转换为迭代器，遍历 batch
            # We chunkify the epoch iterator into gradient accumulation steps `n` batches
            remainder = num_examples % args.gradient_accumulation_steps
            if remainder == 0:
                remainder = args.gradient_accumulation_steps
            update_step = -1
            total_updates = steps_in_epoch // args.gradient_accumulation_steps + 1 # 计算一个 epoch 内的总更新步数
            if args.gradient_accumulation_steps == 1:
                total_updates -= 1
            # 开始batch训练
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # print(inputs.keys())
        # print(inputs['attention_mask'].shape, inputs['num_items_in_batch'])
...
class Hotmoe_Trainer(Trainer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    
    def gen_batch_inputs(self, batch_samples):
        batch_inputs = defaultdict(list)

        for mini_batch in batch_samples:
            for key, value in mini_batch.items():
                batch_inputs[key].append(value)

        batch_inputs = {key: torch.cat(value, dim=0) if isinstance(value[0], torch.Tensor) else value
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for _ in range(total_updates):
                ...
                num_batches = args.gradient_accumulation_steps if update_step != (total_updates - 1) else remainder
                batch_samples, num_items_in_batch = self.get_batch_samples(epoch_iterator, num_batches)
                # batch_inputs=self.gen_batch_inputs(batch_samples)
                for i, inputs in enumerate(batch_samples):
                    step += 1
                    do_sync_step = (step + 1) % args.gradient_accumulation_steps == 0 or (step + 1) == steps_in_epoch
                    # Since we perform prefetching, we need to manually set sync_gradients
                    ...
                    with context():
                        if i == len(batch_samples) - 1: # last batch
                            tr_loss_step = self.training_step(model, inputs, num_items_in_batch, last_minibatch=True)
                        else:
                            tr_loss_step = self.training_step(model, inputs, num_items_in_batch)
    ...
    def training_step(
        self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], num_items_in_batch=None, last_minibatch=False
    ) -> torch.Tensor:
        """
        Perform a training step on a batch of inputs.
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None, last_minibatch=False):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.
        ...
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            loss = model.module.auxloss_functions.get_auxiliary_loss(loss, inputs, last_minibatch=last_minibatch)
        else:
            loss = model.auxloss_functions.get_auxiliary_loss(loss, inputs, last_minibatch=last_minibatch)
